...
public:
	...
	inline void feedback(const float32_t &fb) 
	{
		...
		feedb = mapf(feedb, 0.0f, 1.0f, 0.1f, feedb_max);
		feedback_tmp = feedb;
		inGain = mapf(feedb, 0.1f, feedb_max, 0.5f, 0.2f);
	...
	// original reverbsc code sets feedback from fb parameter so thats what we do here 
	//	float32_t inGain;
	//	float32_t feedb = 2.0f * fb - fb*fb;
